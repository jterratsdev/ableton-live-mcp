...
assert.match(bridgeSource, /transport_command_response\(False, song\.is_playing\)/);
assert.match(bridgeSource, /"requestedPlaying": requested/);
assert.match(bridgeSource, /"observedPlaying": observed/);
assert.match(bridgeSource, /"confirmed": observed == requested/);
...
assert.match(liveMixerSource, /write_verified_db\(mixer\.volume, db, tolerance_db\)/);
assert.doesNotMatch(liveMixerSource, /db_to_live_volume/);
...
assert.match(liveMeterCacheSource, /"meterDiagnostics"/);
assert.match(liveMeterCacheSource, /"directRaw"/);
assert.match(liveMeterCacheSource, /"cachedRaw"/);
assert.match(liveMeterCacheSource, /meter_object_path/);
assert.match(liveMixerSource, /"meterDiagnostics": snapshot\.get\("meterDiagnostics"\)/);
