# Runtime Child Prompt: ableton-bridge-minimal-20260716

- Runtime: codex-cli
- Task id: ableton-bridge-minimal-20260716
- Phase: developer
- Role: developer
- Session id: ableton-bridge-minimal-20260716:wfrun-1784240094860-3dbb0c:developer:codex-cli
- Direct provider API calls: forbidden

## Goal
Create a minimal local HTTP bridge implementation with adapter boundaries for Ableton control and development-mode behavior for status, project, tempo, transport, and plugin inventory endpoints.

## Acceptance Criteria
- A local bridge server can listen on 127.0.0.1:9789 or a configured loopback port.
- The bridge implements /status, /project, /tempo, /transport/start, /transport/stop, and /plugins according to docs/ableton-bridge-contract.md.
- The bridge has a clear adapter seam for real Ableton integration and a deterministic development adapter for local tests.
- Tests verify the MCP server can call the bridge without ABLETON_MCP_DRY_RUN for status, tempo, transport, and plugins.
...
- original: 344 lines / 16110 bytes
- final: 289 lines / 15006 bytes
...
- finalSize: 0 lines / 0 bytes
- omittedContentWarning: Source path was unavailable during preprocessing: bridge.
- reason: Delegated ownership path; cite this path instead of loading raw file content by default.

Missing or unresolved source path; cite the path and request full context if required.
...
- originalSize: 105 lines / 3333 bytes
- finalSize: 44 lines / 1818 bytes
- omittedContentWarning: Reduced from 105 lines using section-extraction; full source remains at README.md.
- reason: Delegated ownership path; cite this path instead of loading raw file content by default.
...
1. `ableton_get_project` to inspect tracks, devices, clips, locators, and mixer state.
2. `ableton_create_snapshot` before destructive or broad edits.
3. `ableton_list_plugins` so the LLM can choose available instruments/effects.
4. `ableton_import_midi`, `ableton_create_midi_clip`, or both.
5. `ableton_select_vst_for_midi` or `ableton_load_device` to assign sounds.
6. `ableton_modify_track`, `ableton_set_device_parameter`, and `ableton_set_automation` for arrangement and mix moves.
7. `ableton_apply_mastering_chain` for master bus preparation.
8. `ableton_export_render` for master, selected tracks, all tracks, or stems.
## MCP Client Config
...
Recommended first Ableton-side implementation:

1. Create a Max for Live device or Ableton Extension SDK extension.
2. Start a local HTTP listener on `127.0.0.1:9789`.
3. Map each endpoint to Live API operations.
4. Return compact JSON responses.
```
...
- strategy: passthrough
- originalSize: 19 lines / 467 bytes
- finalSize: 19 lines / 466 bytes
- omittedContentWarning: none
- reason: Delegated ownership path; cite this path instead of loading raw file content by default.
...
```
{
  ...
  "bin": {
    "ableton-live-mcp": "./src/server.js"
  },
  "scripts": {
    "start": "node src/server.js",
    "smoke": "node test/smoke.mjs",
    "test": "node test/smoke.mjs && node test/regression.mjs"
  },
  "engines": {
    "node": ">=18"
  },
...
- Load budget: normal
- Rationale: eligible task roles: developer; task text matches triggers: plugin, plugins
- Instructions:
...
- Load budget: small
- Rationale: eligible task roles: developer; task text matches triggers: docs, verify
- Instructions:
...
- Load budget: small
- Rationale: eligible task roles: developer; task text matches triggers: docs; task touches files, so prompt registry can preserve artifact intent
- Instructions:
...
- Load budget: normal
- Rationale: eligible task roles: developer; task text matches triggers: integration
- Instructions:
...
- Load budget: normal
- Rationale: eligible task roles: developer; task text matches triggers: test
- Instructions:
...
- Load budget: normal
- Rationale: task text matches triggers: docs, readme
- Instructions:
...
- Load budget: normal
- Rationale: task text matches triggers: pr
- Instructions:
...
## Return Contract
- Write or summarize result evidence for .agent-workflow/handoffs/ableton-bridge-minimal-20260716-wfrun-1784240094860-3dbb0c-developer-developer-runtime-handoff.md.
...
- After writing .agent-workflow/handoffs/ableton-bridge-minimal-20260716-wfrun-1784240094860-3dbb0c-developer-developer-runtime-handoff.md, run: orchestra runtime spawn-lifecycle --session ableton-bridge-minimal-20260716:wfrun-1784240094860-3dbb0c:developer:codex-cli --status completed --agent-id codex-cli:child --artifact .agent-workflow/handoffs/ableton-bridge-minimal-20260716-wfrun-1784240094860-3dbb0c-developer-developer-runtime-handoff.md
- If blocked or failed before completion, run: orchestra runtime spawn-lifecycle --session ableton-bridge-minimal-20260716:wfrun-1784240094860-3dbb0c:developer:codex-cli --status failed --agent-id codex-cli:child --summary "<safe failure summary>"
