# Runtime Child Prompt: ableton-release-0-2-0-scene-p0-remediation-20260823
...
## Goal
Remove the known Scene transaction receiver-isolation P0 from the 0.2.0 release candidate and prove the correction across Node and Python runtimes.

## Acceptance Criteria
- Node and Python adversarial tests prove a same-name same-shape replacement Scene receives zero forward and rollback setters after transaction start.
- Every forward and compensation setter uses only the exact Scene receiver captured during preflight; fresh sceneIndex resolution is observation-only and cannot authorize mutation.
- Focused scene contract tests, Python compilation, capability parity checks, full npm test, package checks, and git diff check pass offline without contacting Ableton.

## Ownership Paths
- bridge/development/scene-tempo-signature.js
- ableton_remote_scripts/AbletonMcpBridge/live_scene_tempo_signature.py
- test/scene-tempo-signature.mjs
- test/live_scene_tempo_signature_test.py
...
- original: 1315 lines / 61703 bytes
- final: 360 lines / 17822 bytes
...
- sourcePath: bridge/development/scene-tempo-signature.js
- strategy: head-tail-truncation
- originalSize: 253 lines / 11874 bytes
- finalSize: 49 lines / 2103 bytes
- omittedContentWarning: Reduced from 253 lines using head-tail-truncation; full source remains at bridge/development/scene-tempo-signature.js.
...
[... omitted 205 lines; full source path available ...]
}
...
- sourcePath: ableton_remote_scripts/AbletonMcpBridge/live_scene_tempo_signature.py
- strategy: head-tail-truncation
- originalSize: 480 lines / 22528 bytes
- finalSize: 49 lines / 1420 bytes
- omittedContentWarning: Reduced from 480 lines using head-tail-truncation; full source remains at ableton_remote_scripts/AbletonMcpBridge/live_scene_tempo_signature.py.
- reason: Delegated ownership path; cite this path instead of loading raw file content by default.
...
[... omitted 432 lines; full source path available ...]
    for value in values:
...
- sourcePath: test/scene-tempo-signature.mjs
- strategy: head-tail-truncation
- originalSize: 362 lines / 14991 bytes
- finalSize: 42 lines / 1990 bytes
- omittedContentWarning: Reduced from 362 lines using head-tail-truncation; full source remains at test/scene-tempo-signature.mjs.
- reason: Delegated ownership path; cite this path instead of loading raw file content by default.
...
[... omitted 321 lines; full source path available ...]
  const node = {
...
- Load budget: normal
- Rationale: eligible task roles: qa; task text matches triggers: pr, diff
- Instructions:
...
- Load budget: small
- Rationale: eligible task roles: developer, qa; task touches files, so prompt registry can preserve artifact intent
- Instructions:
...
- Load budget: normal
- Rationale: task text matches triggers: release, rollback
- Instructions:
...
- Load budget: small
- Rationale: eligible task roles: developer, qa; baseline source selection should run for every task
- Instructions:
...
## Return Contract
- Write or summarize result evidence for .agent-workflow/handoffs/ableton-release-0-2-0-scene-p0-remediation-20260823-wfrun-1787519903534-0e56ce-developer-developer-runtime-handoff.md.
...
- After writing .agent-workflow/handoffs/ableton-release-0-2-0-scene-p0-remediation-20260823-wfrun-1787519903534-0e56ce-developer-developer-runtime-handoff.md, run: orchestra runtime spawn-lifecycle --session ableton-release-0-2-0-scene-p0-remediation-20260823:wfrun-1787519903534-0e56ce:developer:codex-cli --status completed --agent-id codex-cli:child --artifact .agent-workflow/handoffs/ableton-release-0-2-0-scene-p0-remediation-20260823-wfrun-1787519903534-0e56ce-developer-developer-runtime-handoff.md
- If blocked or failed before completion, run: orchestra runtime spawn-lifecycle --session ableton-release-0-2-0-scene-p0-remediation-20260823:wfrun-1787519903534-0e56ce:developer:codex-cli --status failed --agent-id codex-cli:child --summary "<safe failure summary>"
