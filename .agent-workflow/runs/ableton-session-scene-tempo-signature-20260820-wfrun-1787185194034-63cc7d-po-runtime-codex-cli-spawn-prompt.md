...
- Phase: po
- Role: product_owner
- Session id: ableton-session-scene-tempo-signature-20260820:wfrun-1787185194034-63cc7d:po:codex-cli
...
## Acceptance Criteria
- A read-only capability response reports separately whether the active Live Scene proxy exposes readable and writable tempo, tempo-enable, time-signature, and time-signature-enable properties, with explicit reasons for unavailable fields.
- An MCP caller can target one exact existing Session scene and set a supported tempo override, time-signature override, or both; the response rereads and returns the exact observed scene values and enable state.
- The mutation contract validates scene identity/index, BPM and signature bounds, requires at least one requested change, fails closed before mutation when required properties are unavailable, and restores the complete observed scene state if a later write or readback verification fails.
- The tool and documentation explicitly describe Session-scene launch behavior and never claim to create Arrangement tempo envelopes or Arrangement time-signature markers.
- Capability-aware tools/list exposes the scene operation only when the active bridge contract supports the required route, while deterministic development fixtures preserve equivalent behavior and malformed capability handshakes fail closed.
- Focused Node and Python fake-Live tests cover success, unsupported properties, invalid targets, partial-write rollback, recreated-proxy readback, clear/disable semantics if supported, registry parity, py_compile, npm test, and git diff --check without contacting the active bridge or mutating a Live Set.
...
- Load budget: small
- Rationale: eligible task roles: developer, qa; task text matches triggers: fixture, fixtures
- Instructions:
...
- Load budget: small
- Rationale: eligible task roles: architect, developer, qa; task text matches triggers: documentation, docs
- Instructions:
...
## Return Contract
- Write or summarize result evidence for .agent-workflow/handoffs/ableton-session-scene-tempo-signature-20260820-wfrun-1787185194034-63cc7d-po-product_owner-runtime-handoff.md.
...
- After writing .agent-workflow/handoffs/ableton-session-scene-tempo-signature-20260820-wfrun-1787185194034-63cc7d-po-product_owner-runtime-handoff.md, run: orchestra runtime spawn-lifecycle --session ableton-session-scene-tempo-signature-20260820:wfrun-1787185194034-63cc7d:po:codex-cli --status completed --agent-id codex-cli:child --artifact .agent-workflow/handoffs/ableton-session-scene-tempo-signature-20260820-wfrun-1787185194034-63cc7d-po-product_owner-runtime-handoff.md
- If blocked or failed before completion, run: orchestra runtime spawn-lifecycle --session ableton-session-scene-tempo-signature-20260820:wfrun-1787185194034-63cc7d:po:codex-cli --status failed --agent-id codex-cli:child --summary "<safe failure summary>"
