# Runtime Child Prompt: ableton-mixer-snapshot-rollback-safety-20260723

- Runtime: codex-cli
- Task id: ableton-mixer-snapshot-rollback-safety-20260723
- Phase: developer
- Role: developer
- Session id: ableton-mixer-snapshot-rollback-safety-20260723:wfrun-1784850980476-b9a293:developer:codex-cli
- Direct provider API calls: forbidden

## Goal
Make project snapshots capture and rollback the complete mixer state for tracks, returns, and master without relying on unsafe dB conversion.

## Acceptance Criteria
- Snapshots capture raw volume, pan, sends, mute, solo, arm where supported, plus return and master mixer state.
- Rollback restores captured mixer values using raw parameter values and reports per-field applied, skipped, and failed results.
- Rollback fails visibly when required state cannot be restored and never reports blanket success for partial restoration.
- Tests cover round-trip restoration of track, return, and master mixer state including the user's restored track levels.
- No live rollback is executed until the user explicitly approves a disposable verification or confirms a backup.
...
- original: 477 lines / 21164 bytes
- final: 322 lines / 16048 bytes
...
- sourcePath: ableton_remote_scripts/AbletonMcpBridge/live_snapshots.py
- strategy: head-tail-truncation
- originalSize: 118 lines / 4964 bytes
- finalSize: 49 lines / 2104 bytes
- omittedContentWarning: Reduced from 118 lines using head-tail-truncation; full source remains at ableton_remote_scripts/AbletonMcpBridge/live_snapshots.py.
...
def project_snapshot(song):
    snapshot = {
        ...
        "coverage": {
            ...
            "returns": {
                "mixerState": True,
[... omitted 70 lines; full source path available ...]
...
- sourcePath: bridge/development/snapshots.js
- strategy: head-tail-truncation
- originalSize: 139 lines / 3890 bytes
- finalSize: 49 lines / 1359 bytes
- omittedContentWarning: Reduced from 139 lines using head-tail-truncation; full source remains at bridge/development/snapshots.js.
- reason: Delegated ownership path; cite this path instead of loading raw file content by default.
...
[... omitted 91 lines; full source path available ...]
    devices: true,
    clips: true,
...
- Load budget: small
- Rationale: eligible task roles: developer; task text matches triggers: ui, docs; task touches files, so prompt registry can preserve artifact intent
- Instructions:
...
- Load budget: small
- Rationale: eligible task roles: developer; task text matches triggers: failed
- Instructions:
...
- Load budget: small
- Rationale: eligible task roles: developer; task text matches triggers: fixture
- Instructions:
...
- Load budget: normal
- Rationale: task text matches triggers: ui
- Instructions:
...
## Return Contract
- Write or summarize result evidence for .agent-workflow/handoffs/ableton-mixer-snapshot-rollback-safety-20260723-wfrun-1784850980476-b9a293-developer-developer-runtime-handoff.md.
...
- After writing .agent-workflow/handoffs/ableton-mixer-snapshot-rollback-safety-20260723-wfrun-1784850980476-b9a293-developer-developer-runtime-handoff.md, run: orchestra runtime spawn-lifecycle --session ableton-mixer-snapshot-rollback-safety-20260723:wfrun-1784850980476-b9a293:developer:codex-cli --status completed --agent-id codex-cli:child --artifact .agent-workflow/handoffs/ableton-mixer-snapshot-rollback-safety-20260723-wfrun-1784850980476-b9a293-developer-developer-runtime-handoff.md
- If blocked or failed before completion, run: orchestra runtime spawn-lifecycle --session ableton-mixer-snapshot-rollback-safety-20260723:wfrun-1784850980476-b9a293:developer:codex-cli --status failed --agent-id codex-cli:child --summary "<safe failure summary>"
