# Runtime Child Prompt: ableton-product-render-export-20260720

- Runtime: codex-cli
- Task id: ableton-product-render-export-20260720
- Phase: developer
- Role: developer
- Session id: ableton-product-render-export-20260720:wfrun-1784583682090-9ba66a:developer:codex-cli
- Direct provider API calls: forbidden

## Goal
Close the product gap around export/render so production workflows can produce inspectable audio artifacts or explicit unsupported results.

## Acceptance Criteria
- Define the product contract for master export, selected-track export, and stem export with explicit Live Remote Script feasibility.
- Add or update deterministic tests for render/export behavior and unsupported Live fallback semantics.
- Document remaining Live API limitations and the next implementation surface if Remote Script cannot render directly.
...
- original: 502 lines / 22547 bytes
- final: 318 lines / 16039 bytes
...
- sourcePath: docs/render-export-plan.md
- strategy: missing
- originalSize: 0 lines / 0 bytes
- finalSize: 0 lines / 0 bytes
- omittedContentWarning: Source path was unavailable during preprocessing: docs/render-export-plan.md.
...
- sourcePath: bridge/development/render.js
- strategy: head-tail-truncation
- originalSize: 116 lines / 4400 bytes
- finalSize: 49 lines / 1977 bytes
- omittedContentWarning: Reduced from 116 lines using head-tail-truncation; full source remains at bridge/development/render.js.
- reason: Delegated ownership path; cite this path instead of loading raw file content by default.
...
export async function exportDevelopmentRender(state, payload = {}) {
  ...
  if (request.scope === "stems") {
    ...
    const tracks = payload.includeReturnTracks === true
[... omitted 68 lines; full source path available ...]
...
- sourcePath: bridge/development/production-workflows.js
- strategy: head-tail-truncation
- originalSize: 166 lines / 5837 bytes
- finalSize: 49 lines / 1753 bytes
- omittedContentWarning: Reduced from 166 lines using head-tail-truncation; full source remains at bridge/development/production-workflows.js.
- reason: Delegated ownership path; cite this path instead of loading raw file content by default.
...
export async function bounceTracks(state, payload = {}) {
  ...
  return {
    ok: true,
[... omitted 118 lines; full source path available ...]
    master: clone(state.master),
    warnings
  };
}

function validateTrackIndices(state, trackIndices) {
...
- sourcePath: test/render-export.mjs
- strategy: missing
- originalSize: 0 lines / 0 bytes
- finalSize: 0 lines / 0 bytes
- omittedContentWarning: Source path was unavailable during preprocessing: test/render-export.mjs.
- reason: Delegated ownership path; cite this path instead of loading raw file content by default.
...
- Load budget: small
- Rationale: eligible task roles: developer; task text matches triggers: artifact, docs; task touches files, so prompt registry can preserve artifact intent
- Instructions:
...
- Load budget: normal
- Rationale: eligible task roles: developer; task text matches triggers: audio
- Instructions:
...
- Load budget: small
- Rationale: eligible task roles: developer; task text matches triggers: workflow
- Instructions:
...
- Load budget: small
- Rationale: eligible task roles: developer; task text matches triggers: docs
- Instructions:
...
- Load budget: normal
- Rationale: task text matches triggers: docs
- Instructions:
...
- Load budget: normal
- Rationale: task text matches triggers: fallback
- Instructions:
...
## Return Contract
- Write or summarize result evidence for .agent-workflow/handoffs/ableton-product-render-export-20260720-wfrun-1784583682090-9ba66a-developer-developer-runtime-handoff.md.
...
- After writing .agent-workflow/handoffs/ableton-product-render-export-20260720-wfrun-1784583682090-9ba66a-developer-developer-runtime-handoff.md, run: orchestra runtime spawn-lifecycle --session ableton-product-render-export-20260720:wfrun-1784583682090-9ba66a:developer:codex-cli --status completed --agent-id codex-cli:child --artifact .agent-workflow/handoffs/ableton-product-render-export-20260720-wfrun-1784583682090-9ba66a-developer-developer-runtime-handoff.md
- If blocked or failed before completion, run: orchestra runtime spawn-lifecycle --session ableton-product-render-export-20260720:wfrun-1784583682090-9ba66a:developer:codex-cli --status failed --agent-id codex-cli:child --summary "<safe failure summary>"
